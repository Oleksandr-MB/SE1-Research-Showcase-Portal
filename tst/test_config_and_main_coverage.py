...
class TestConfigAndMainCoverage(unittest.TestCase):
    def test_read_config_private_missing_optional_returns_default(self):
        with patch.dict(os.environ, {}, clear=True):
            self.assertEqual(read_config(required=False), {})
            self.assertEqual(read_config(required=False, default={"x": 1}), {"x": 1})
    ...
    def test_read_config_public_env_overrides_defaults(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            cfg = read_config()
        self.assertEqual(cfg["RSP_DB_HOST"], "db.example")
        self.assertEqual(cfg["RSP_DB_PORT"], 5433)
        self.assertEqual(cfg["RSP_EMAIL_LINK_BASE"], "https://front/verify-email?token=")
